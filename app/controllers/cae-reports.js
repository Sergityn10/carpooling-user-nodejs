...
const getReportsSummary = catchAsync(async (req, res, next) => {
  ...
  if (!data) {
    return next(
      new AppError(
        "Error fetching summary from trayectos service",
        502,
...
const listReports = catchAsync(async (req, res, next) => {
  const adminToken = getAdminToken(req);
  if (!adminToken) {
    return next(
      new AppError(
  ...
  if (!data) {
    return next(
      new AppError(
        "Error fetching reports from trayectos service",
        502,
        "TRAYECTOS_SERVICE_UNAVAILABLE",
      ),
    );
...
const createReport = catchAsync(async (req, res, next) => {
  ...
  if (!data) {
    return next(
      new AppError(
        "Error creating report in trayectos service",
        502,
        "TRAYECTOS_SERVICE_UNAVAILABLE",
      ),
    );
...
const getReportById = catchAsync(async (req, res, next) => {
  ...
  if (!reportData) {
    return next(
      new AppError(
        "Error fetching report from trayectos service",
        502,
        "TRAYECTOS_SERVICE_UNAVAILABLE",
      ),
    );
...
const exportReportExcel = catchAsync(async (req, res, next) => {
  const adminToken = getAdminToken(req);
  if (!adminToken) {
    return next(
      new AppError(
  ...
  if (items.length === 0) {
    return next(
      new AppError("No CAEs found in this report", 404, "REPORT_EMPTY"),
    );
  }
...
const updateReportStatus = catchAsync(async (req, res, next) => {
  ...
  if (!status) {
    return next(
      new AppError("status field is required", 400, "VALIDATION_ERROR"),
    );
  }
  ...
  if (!data) {
    return next(
      new AppError(
        "Error updating report status in trayectos service",
        502,
        "TRAYECTOS_SERVICE_UNAVAILABLE",
      ),
    );
...
const deleteReport = catchAsync(async (req, res, next) => {
  ...
  if (!data) {
    return next(
      new AppError(
        "Error deleting report in trayectos service",
        502,
        "TRAYECTOS_SERVICE_UNAVAILABLE",
      ),
    );
