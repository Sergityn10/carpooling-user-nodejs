...
async function createUser({ username, email, password, name }, auth_method='password', google_id=''){
    ...
    switch(auth_method){
        ...
        case 'google':
            const googleResult = await database.execute({
                ...
                args: [username, email, hash, name, customer_account.id, auth_method, google_id]
            });
